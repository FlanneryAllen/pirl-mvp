...
type Artifact = {
  id: string;
  title: string;                 // e.g., "Grow a Garden PIRL Seed"
  created_at: string;
  mission_id: string | null;
  missions: { title: string; slug: string } | null;
};

/** Decide icon + colors + action by artifact title keywords */
...
export default function Lockr() {
  ...
  useEffect(() => {
    (async () => {
      ...
      // 1) Fetch unlocked artifacts for this kid
      ...
      // 2) Count how many players earned each reward (by mission_id)
    ...
    <div className="p-6 max-w-md mx-auto text-white">
      ...
      <div className="grid grid-cols-2 gap-3">
        {discoverables.map((r, i) => (
          <div
            key={i}
            className="rounded-xl border border-white/10 bg-white/5 p-4 flex flex-col items-center justify-center text-center text-white/65"
          >
